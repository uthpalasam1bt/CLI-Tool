...
export const OnSubmitHook = {
    ...
    //     pickerValidator(dataset, 'signatories', 'FMA', [{alias: 'client', errorMessage: 'Please select signatories.'}]),
    ...
    whenSubmitValidation: (formData, asyncErr, dataset) => {
        ...
        if (noTrusteeEntities)
            return {
                message: `At least ${minCorpTrustees} ${trusteePlaceholder} to continue.`
            };
        ...
        if (trusteeEntities.length > 0) {
            for (const entity of trusteeEntities) {
                for (const [i, trustee] of _.get(entity, 'trustees', []).entries()) {
                    if (_.get(trustee, 'signDigitally', 'no') == 'no' && _.has(trustee, 'phoneNumber'))
            ...
            copyFormData ['trusteeEntities'] = trusteeEntities;
                        ...
                        if (Object.keys(newOtps) && Object.keys(newOtps).length) {
                            Modal.confirm({
                                ...
                                onCancel() {
                                    ...
                                    store.dispatch(
                                        change(FORM_NAME, `trusteeEntities`, trusteeEntities)
