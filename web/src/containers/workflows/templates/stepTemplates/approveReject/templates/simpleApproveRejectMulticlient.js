...
const SimpleApproveRejectMulticlient = props => {
    const {
        dataset,
    ...
    const [documentName, setDocumentName] = useState(_documentName);

    useEffect(() => {
        if (dataset && dataset.formData && dataset.formData.selectedDocType) {
            if (dataset.formData.selectedDocType === 'FMA_DOC') {
                setDocumentName('FMA');
            } else if (dataset.formData.selectedDocType === 'IAA_DOC') {
                setDocumentName('IAA');
            }
        }
    }, [JSON.stringify(_.get(dataset, `formData.selectedDocType`))]);
    ...
    const onConfirmModalSubmit = ({ comment }) => {
        if (dataset && loggedUser) {
            ...
            _.set(_dataset, `approvers.${documentName}.users`, [
                {
                    ...approver,
                    status: userAction
                }
                ...
                ? [
                      {
                          ...
                          onClick: () => {
                            ...
                            // claimHelper.getPermission(props.isMultiClient ? commonUserClaims : getLoggedUserClaims_data, step, BUTTON_CLAIM_DOWNLOAD)
                      }
                  ]
                : []),
            ...(rejectButton
                ? [
                      {
                          ...
                          bool:
                              ...
                              !step.rejected &&
                              !hasRejected() &&
                              claimHelper.getPermission(props.isMultiClient ? commonUserClaims : getLoggedUserClaims_data, step, STEP_ACTION_AUTHORIZE),
...
)(SimpleApproveRejectMulticlient);
