...
const exisitingWorkflows=getDirectoriesFormThePath(`../web/src/containers/schemeOptions/updates`)
    ...
    try {
        ...
        if(answers && answers.workflow && answers.stepCount){
            if(answers.workflow){
                ...
                fs.mkdirSync(path);
                console.log(chalk.green(`workflow ${answers.workflow} has been created ${path}`))
            }
